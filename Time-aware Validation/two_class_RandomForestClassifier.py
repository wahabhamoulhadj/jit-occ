# Binary Random Forest applied to the prediction of risky commits
...
def perform_grid_search(x_train, x_validation, max_depths, balancing_method=None):
    label = "isBug"
    best_auc = 0.0
    best_model = RandomForestClassifier(random_state=2018)
    target_class = 1
    ...
    with warnings.catch_warnings():
        ...
        for d in max_depths:
            # Train the model
            model = RandomForestClassifier(max_depth=d, max_features=13, random_state=2018, min_samples_leaf=5)
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", category=ConvergenceWarning)
...
def plot_roc_curve(y_test, y_scores, y_predict, f1, mcc, project_name):
    ...
    plt.grid(axis='both')
    plt.savefig(f"./figures/TV/RF_{project_name}.png")
    plt.clf()
...
def update_features(feature_importance_normalized, feature_names, feature_names_dic):
...
def start(file_path: str, balancing_method=None, display=False):
    ...
    for test_case in range(1, 4):
        ...
        feature_importance_normalized = np.mean([tree.feature_importances_ for tree in
                                                ...
                                                axis=0)
        feature_names_dic = update_features(feature_importance_normalized, feature_names, feature_names_dic)
    ...
    plt.xlabel("FPR")
    plt.ylabel("TPR")
    plt.savefig(f"./figures/TV/RF_{project_name}_{balancing_method}.png")
    if display:
        plt.show()
    plt.clf()
...
if __name__ == '__main__':
    if not test_mode:
        ...
        for code_metric in code_metrics:
            ...
            report.to_csv(f"./results/final_report_RF_Binary_TV_{code_metric}_{balance}_bootstrap.csv", index=None, header=True)
