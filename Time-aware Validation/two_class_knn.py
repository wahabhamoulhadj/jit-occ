# Binary k-NN applied to the prediction of risky commits
...
def perform_grid_search(x_train, x_validation, ks, balancing_method=None):
    label = "isBug"
    best_auc = 0.0
    best_model = KNeighborsClassifier(algorithm="kd_tree", weights="distance",n_neighbors=1)
    target_class = 1
    ...
    with warnings.catch_warnings():
        ...
        for k in ks:
            if len(y_train) < k:
                continue
            # Train the model
            model = KNeighborsClassifier(algorithm="kd_tree", n_neighbors=k, weights="distance")
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", category=ConvergenceWarning)
                ...
                print(f"Current AUC {auc:.5f}, ", end="\r")
                if best_auc < auc:
                    best_auc = auc
                    best_model = model
                    print(f"Best AUC = {best_auc:.3f}",end='\r')
    print(f"{best_model}")
    return best_model
...
def plot_roc_curve(y_test, y_scores, y_predict, f1, mcc, project_name):
    ...
    plt.savefig(f"./figures/TV/kNN_{project_name}.png")
    # plt.show()
    plt.clf()
...
def start(file_path: str, balancing_method=None, display=False):
    ...
    n_neighbors = [x for x in range(15, 50, 2)]
    ...
    for test_case in range(1, 4):
        ...
        best_model = perform_grid_search(train_sample, validation_sample, n_neighbors,
                                         balancing_method=balancing_method)
        # Train the knn model with the best values
        model = best_model
        if model == None:
            continue
    ...
    plt.xlabel("FPR")
    plt.ylabel("TPR")
    plt.savefig(f"./figures/TV/KNN_{project_name}_{balancing_method}_bootstrap.png")
    if display:
        plt.show()
    plt.clf()
    ...
    if not test_mode:
        ...
        for code_metric in code_metrics:
            ...
            frame = {"Project name": projects,
                     # "ir_training": ir_training,
                     # "ir_testing": ir_testing,
                     "f1 score": f1_s,
                     "mcc": mcc_s,
                     "auc": auc_s,
                     "f1 score STD": f1_ci,
            ...
            report.to_csv(f"./results/final_report_KNN_Binary_TV_{code_metric}_{balance}_bootstrap.csv", index=None, header=True)
