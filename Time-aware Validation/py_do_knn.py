# One class k-NN applied to the prediction of risky commits
...
ir_training = []  # Imbalanced ratio
ir_testing = []  # Imbalanced ratio
cv_s = []
buggies = []
normals = []
...
def perform_hyper_turning_search(x_train, x_validation, n_neighbors, knn_metrics, methods):
    label = "isBug"
    best_auc = 0.0
    best_model = KNN(algorithm='auto')
    target_class = 1
    ...
    for n in n_neighbors:
        for method in methods:
            for metric in knn_metrics:
                ...
                predictions = model.predict(validation_commits_features)
    ...
    # We need to merge the train and validate data here....
...
def evaluate_ocknn(model, X_test, y_test):
...
def plot_roc_curve(y_test, y_scores, project_name, display=False):
    ...
    # plt.scatter(fpr, tpr, c=thresholds, cmap='RdYlBu', label='Thresholds')
    # plt.plot([0, 1], [0, 1], 'k--')
    # Plot the best F1-score and MCC points
    # best_f1_index = np.argmax(f1)
    # best_mcc_index = np.argmax(mcc)
    # plt.scatter(fpr[best_f1_index], tpr[best_f1_index], color='green', marker='o', s=20,
    #             label='Best F1-score %0.2f' % f1[best_f1_index])
    # plt.scatter(fpr[best_mcc_index], tpr[best_mcc_index], color='blue', marker='o', s=20,
    #             label='Best MCC %0.2f' % mcc[best_mcc_index])
    ...
    plt.savefig(f"./figures/TV/OCKNN_{project_name}.png")
    if display:
        plt.show()
    plt.clf()
...
def start(file_path: str, display=False):
    ...
    # if project_name in ['ambari', 'bookkeeper', 'hive']:
    #     test_size = 0.65
    # else:
    test_size = 0.30
    ...
    method = ['mean', 'largest', 'median']
    n_neighbors = [x for x in range(5, 30, 2)]
    ...
    for test_case in range(1, 31):
        ...
        auc_roc, f1, mcc, fpr, tpr = evaluate_ocknn(best_model, testing_set_features, testing_set_targets)
    ...
    # plot the ROC
    # Plot the ROC curve with best F1-score and MCC
    # y_scores = best_model.predict_proba(testing_set_features)[:, target]
    # plot_roc_curve(testing_set_targets, y_scores, project_name, display=False)
...
if __name__ == '__main__':
    if not test_mode:
        ...
        for code_metric in code_metrics:
            ...
            report.to_csv(f"./results/final_report_oc_knn_TV_{code_metric}_bootstrap.csv", index=None, header=True)
            print("Collecting...")
