    ...
    for test_case in range(1, 11):  # here we need to run it 10 times
        ...
        for train_index, validate_index in kf.split(commits_healthy):
            ...
            for k in range(1, 20, 2):
                for m in ['mean', 'largest', 'median']:
                    # for met in ['euclidean', 'minkowski','manhattan']:
                        knn = KNN(n_neighbors=k, algorithm='auto', method=m, metric='euclidean')
                        # Train model with healthy only
                        knn.fit(train_healthy_commits)
                        predictions = knn.predict(validation_commits_features)
                        predictions_proba = knn.predict_proba(validation_commits_features)
            ...
            print(f"Fold# {fold_id} , best parameters k = {fold_k},best method = {best_m}, metric = {best_met}, MCC = {best_mcc} and AUC {auc_fold}")
            fold_id = fold_id + 1
        ...
        print(f"Best parameters {best_k}, AUC {best_auc} and MCC = {best_mcc}")
        ...
        knn = KNN(n_neighbors=best_k, method=best_m ,algorithm='auto', metric=best_met)
        ...
        knn.fit(train_best_model_data)
        # 5. Test the model using 20% of healthy commits and 90% of buggy commits
        predictions = knn.predict(testing_set_features)
        predictions_proba = knn.predict_proba(testing_set_features)
...
if __name__ == '__main__':
    if not test_mode:
        ...
        for code_metric in code_metrics:
            ...
            report.to_csv(f"./results/final_report_oc_knn_{code_metric}.csv", index=None, header=True)
            ...
            report.to_csv(f"./results/final_validation_report_knn_{code_metric}.csv", index=None, header=True)
            print("Collecting...")
        ...
        start('dataset/NET/ant-1.4--NET.csv')
    exit(0)
