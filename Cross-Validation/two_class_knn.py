...
def start(file_path: str, balancing_method=None):
    ...
    for test_case in range(1, 31):  # no need for this
        ...
        for train_index, validate_index in kf.split(tuning_features, tuning_targets):
            ...
            model_name = 'kNN Binary Classifier'
            fold_k = 1
            mcc_fold = -1
            auc_fold = -1
            best_fpr = best_tpr = 0
            max_sample = len(train_commits_targets)
            if max_sample >= 21:
                max_sample = 21
            for k in range(5, 21, 2):
                ...
                knn.fit(train_commits_features, train_commits_targets)
        ...
        print(f"Best parameters k = {best_k}, MCC = {best_mcc} and AUC {best_auc}")
        # AUC
        ...
        knn.fit(tuning_features, tuning_targets)
...
if __name__ == '__main__':
    if not test_mode:
        ...
        for code_metric in code_metrics:
            ...
            report.to_csv(f"./results/final_report_KNN_Binary_{code_metric}_{balance}.csv", index=None, header=True)
            ...
            report.to_csv(f"./results/final_validation_report_KNN_Binary_{code_metric}.csv", index=None, header=True)
            print("Collecting...")
